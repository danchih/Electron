...
function checkAndInsertAdmin() {
  ...
  connection.query('SELECT * FROM usuarios WHERE email = ?', [emailToCheck], (error, results) => {
    ...
    if (results.length > 0) {
      ...
      const sqlInsert = "INSERT INTO usuarios (nome, email, senha, cep, rua, bairro, localidade, uf) VALUES ('admin', 'admin@exemplo.com', 'senhaadmin', 13033195, rua_ex, camp, sp, br)";
